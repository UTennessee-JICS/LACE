...
MACHINE="beacon"
DATE=$(date +'%Y%m%d')
DIR=log_${RUNNAME}_${MACHINE}_${DATE}_${REPETITION}
#threads=(1 16 32)
      ...
      for maxiter in ${GMRESMAXITER[@]}; do
        ...
        for gtol in ${GMRESRELTOL[@]}; do
          ...
          for ptol in ${REDUCTION[@]}; do
            echo REDUCTION = ${ptol}
            LOGFILE=${OUTDIR}log_${MATRIX}_${RHS}_${thread}_${restart}_${maxiter}_${gtol}_ParilU0v02_${ptol}.m
            echo writing to ${LOGFILE}
            ${precommand} ./test_solve_pariLU0_MKL_FGMRES \
            ${MATRIXFILE} ${RHSFILE} ${OUTDIR} ${scale} ${gtol} ${restart} \
