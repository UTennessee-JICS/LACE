  ...
  int **       J_)
{
    ...
    mm_is_sparse(matcode)))
  {
    ...
    fprintf(stderr, "%% Sorry, DEV-sparse does not support ");
  ...
  *nz_ = nz;
  ...
  // CHECK( data_index_malloc_cpu( &I, nz ) );
  // CHECK( data_index_malloc_cpu( &J, nz ) );
  ...
  *J_   = J;
  ...
  /* NOTE: when reading in dataTypes, ANSI C requires the use of the "l"  */
...
int
mm_read_mtx_crd_size(FILE * f, int * M, int * N,
  int * nz)
{
  data_int_t info = 0;
...
int
mm_read_mtx_crd_data(FILE * f, int M, int N, int nz,
  int I[], int J[], dataType val[], MM_typecode matcode)
{
  data_int_t info = 0;
...
mm_read_mtx_crd_entry(FILE * f, int * I, int * J,
  dataType * real, dataType * imag, MM_typecode matcode)
{
  data_int_t info = 0;
...
int
mm_read_mtx_crd(char * fname, int * M, int * N,
  int * nz, int ** I,
  int ** J, dataType ** val, MM_typecode * matcode)
{
  data_int_t info = 0;
  ...
  *val = NULL;
  ...
  if (mm_is_complex(*matcode)) {
    // CHECK( data_dmalloc_cpu( val, *nz*2 ) );
    // *val = (dataType*) malloc( *nz*2*sizeof(dataType) );
    LACE_CALLOC(*val, *nz * 2);
...
int
mm_write_mtx_crd(char fname[], int M, int N, int nz,
  int I[], int J[], dataType val[], MM_typecode matcode)
{
  char buffer[ 1024 ];
  data_int_t info = 0;
...
void
mm_snprintf_typecode(char * buffer, size_t buflen, MM_typecode matcode)
{
  const char * types[4];
