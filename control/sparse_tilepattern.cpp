  ...
  data_d_matrix * A)
{
  ...
  for (int i = 0; i < int(tiles->size()); i++) {
    ...
    int infol = data_sparse_subsparse_cs_lowerupper_handle(sub_m, span, ti, (*tiles)[i].a[2], MagmaLower, A, &Lsub,
        &Lhandle);
    printf("%d L added\n", i);

    data_d_matrix Usub      = { Magma_CSR };
    sparse_matrix_t Uhandle = NULL;
    int infou = data_sparse_subsparse_cs_lowerupper_handle(span, sub_m, (*tiles)[i].a[2], tj, MagmaUpper, A, &Usub,
        &Uhandle);
    printf("%d U added\n", i);
...
} // data_sparse_tilepattern_handles
...
} // data_sparse_tilepatterns_handles
