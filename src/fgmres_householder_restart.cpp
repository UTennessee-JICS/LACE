  ...
  data_d_gmres_log * gmres_log)
{
  ...
  printf("%% data_fgmres_householder restart_max = %d search_max =%d\n",
  ...
  #endif // if (defined(__INTEL_COMPILER) || defined(__ICC))
    ...
    for (int search = 0; search < search_max; ++search) {
      ...
      if (gmres_par->user_csrtrsv_choice == 0) {
        ...
        for (int i = 0; i < Minvvj.ld; i++) {
          Minvvj.val[idx(i, search, Minvvj.ld)] = q.val[i];
        }
      }
      ...
      for (int i = 0; i < n; i++) {
        #pragma nounroll_and_jam
        for (int j = A->row[i]; j < A->row[i + 1]; j++) {
          krylov.val[idx(i, search1,
          krylov.ld)] =
            krylov.val[idx(i, search1, krylov.ld)] + A->val[j] * Minvvj.val[idx(A->col[j], search, Minvvj.ld)];
        }
      }
        ...
        #pragma omp parallel
        {
          ...
          for (int ii = startStrip; ii < endStrip; ii += STRIP) {
            ...
            for (int jj = ii; jj < ii + STRIP; ++jj) {
              krylov.val[idx(jj, search1,
              krylov.ld)] = krylov.val[idx(jj, search1, krylov.ld)] - sum * krylov.val[idx(jj, j, krylov.ld)];
            }
          }
          #pragma omp single
          {
      ...
      if (search < n) {
        GMRESDBG("%e %e %e %e %e\n", krylov.val[idx(search, search1,
          krylov.ld)], -dd, eps, givens_cos.val[search],
          givens_sin.val[search])
        // form search-th rotation matrix
        givens_rotation(krylov.val[idx(search, search1, krylov.ld)], -dd, eps, &givens_cos.val[search],
          &givens_sin.val[search]);
        for (int i = 0; i < n; ++i) {
        ...
        krylov.val[idx(search, search1,
        krylov.ld)] = givens_cos.val[search] * krylov.val[idx(search, search1, krylov.ld)]
