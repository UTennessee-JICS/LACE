...
//    This port to GPU is a partial conversion using only CUBLAS and CUSPARSE routines.
//    Remaining work involves custom kernels to move additional operations to GPU and
    ...
    data_d_gmres_log *gmres_log )
{
    ...
    cudaCheck(cudaMalloc((void**)&d_precondq_val,(n*search_max)*sizeof(dataType)));   
#endif
...
//cublasGetVector(n ,sizeof(dataType), &d_Minvvj_val[idx(0,search,Minvvj.ld)],1, &Minvvj.val[idx(0,search,Minvvj.ld)],1); 
	...
	//ptrsv_tol, &ptrsv_iter );
      ...
      //cublasGetVector(n,sizeof(dataType),&d_krylov_val[search1*krylov.ld],1,&krylov.val[search1*krylov.ld],1);
      cublasCheck(cublasGetVector(n*(search_max+1),sizeof(dataType),d_krylov_val,1,krylov.val,1));
      //cublasGetVector(n,sizeof(dataType),&d_Minvvj_val[search*Minvvj_ld],1,&Minvvj.val[search*Minvvj_ld],1);
      cublasCheck(cublasGetVector(n*search_max,sizeof(dataType),d_Minvvj_val,1,Minvvj.val,1));

      #pragma omp parallel for
        for ( int i=0; i<n; i++ ) {
          for ( int j=A->row[i]; j<A->row[i+1]; j++ ) {
            krylov.val[i+search1*krylov.ld] = krylov.val[i+search1*krylov.ld] + A->val[j]*Minvvj.val[A->col[j]+search*Minvvj.ld];
          }
        }

	//cublasSetVector(n,sizeof(dataType),&krylov.val[search1*krylov.ld],1,&d_krylov_val[search1*krylov.ld],1);
	...
	cublasCheck(cublasSetVector(1,sizeof(dataType),&d_scalar,1, &d_krylov_val[idx(search1,search1,krylov_ld)],1));
        ...
        for (int j=search1; j>=0; --j) {
          dataType sum = 0.0;
          span=n-j;
          cublasCheck(cublasDdot(handle, span, &d_qval[j],1, 
      ...
      cublasCheck(cublasGetVector(n*search_max,sizeof(dataType), d_precondq_val,1, precondq.val,1));
      ...
      for( int j=0; j<search; ++j ) {
        ...
        int span=search;
	cublasCheck(cublasDrot(handle,1, &d_krylov_val[idx(j,search1,krylov_ld)],1,
...
//krylov.val[idx(search,search1,krylov.ld)] = tmp;
 ...
 cublasCheck(cublasGetVector(n*(search+1), sizeof(dataType), d_krylov_val,1, krylov.val,1)); 
	...
	cublasCheck(cublasSetVector(search,sizeof(dataType),alpha.val,1,d_alpha_val,1));        
...
//cublasSetVector(n*search_max,sizeof(dataType),Minvvj.val,1,d_Minvvj_val,1);
	  ...
	  cublasCheck(cublasDaxpy(handle, n, &d_scalar, &d_Minvvj_val[0+j*Minvvj.ld],1,d_zval,1));
          cublasCheck(cublasSetVector(1,sizeof(dataType),&alpha.val[j],1,&d_alpha_val[j],1));        
        }
#endif
