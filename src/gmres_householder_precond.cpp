...
{
  printf("%% data_gmres_householder_precond begin\n");
  ...
  for (int i = 0; i < n; ++i) {
    q.val[i] = -r.val[i] / dd;
    precondq.val[idx(i, 0, h.ld)] = q.val[i];
  }
  for (int i = 0; i < n; ++i) {
    GMRESDBG("q.val[%d] = %e\n", i, q.val[i]);
  }
  ...
  for (int search = 0; search < search_max; search++) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = A->row[i]; j < A->row[i + 1]; j++) {
        ...
        krylov.ld)] = krylov.val[idx(i, search1, krylov.ld)] + A->val[j] * Minvvj.val[A->col[j]];
      }
    }
    ...
    for (int j = 0; j <= search; ++j) {
      ...
      for (int jj = j; jj < n; ++jj) {
        ...
        krylov.ld)] = krylov.val[idx(jj, search1, krylov.ld)] - 2.0 * sum * krylov.val[idx(jj, j, krylov.ld)];
      }
    }
    for (int j = 0; j <= search1; ++j) {
      for (int i = 0; i < n; ++i) {
    ...
    printf("PGMRES_householder_search(%d) = %.16e;\n", search1, residual);
    // update the solution
