...
extern "C"
void
data_PariLU_v3_8( data_d_matrix* A, data_d_matrix* L, data_d_matrix* U, int tile )
{
  ...
  data_sparse_tilepattern_handles( tile, tile, &tiles, &L_subs, &U_subs, &L_handles, &U_handles, A );
  printf( "\n%%A's %d active tiles:\n", int(tiles.size()) );
  ...
  while ( step > tol && iter < 10000 ) {
    ...
    #pragma omp parallel private(ii, jj, ti, tj, maxrow, maxcol, tmp, span, sum )
    {
      ...
      for(int t=0; t< int(tiles.size()); t++ ) {
        ...
        span = MIN( (maxrow - tiles[t].a[2]), (maxcol - tiles[t].a[2]) );
        //printf("==== ti=%d tj=%d span=%d\n", ti, tj, span);

        data_d_matrix C = {Magma_CSR};
        //data_sparse_subsparse_spmm( tile, span, ti, tj, &tiles[i], L, U, &C );
        data_sparse_subsparse_spmm_handle( tile, span, ti, tj, &tiles[t], &L_handles[t], &U_handles[t], &C );
        ...
        if (ti<=tj) {
          ...
          for(int i=ti; i < maxrow; i++ ) {
              ...
              if (U->col[j] >= tj && U->col[j] < maxcol && U->col[j] != i) {
                ...
                for(int k=C.row[ii]; k < C.row[ii+1]; k++ ) {
                  if ( C.col[k] == jj ) {
                    ...
                    U->val[ j ] = tmp;
                    //U_subs[t].val[ j ] = tmp; // correct index
                    //for(int tk=U_subs[t].row[ii]; tk < U_subs[t].row[ii+1]; tk++ ) {
                    //  if ( U_subs[t].col[tk] == (U->col[j] - tj) ) {
                    //    U_subs[t].val[ tk ] = tmp;
                    //  }
                    //}
                  }
            ...
            // U
            //for(int i=tiles[t].a[2]; i<maxrow; i++) {
          ...
          if (ti==tj) {
            ...
            for(int i=ti; i < maxrow; i++ ) {
              for(int j=Lc.row[i]; j < Lc.row[i+1]; j++ ) {
                if (L->col[j] >= tj && L->col[j] < maxcol  && L->col[j] != i) {
                  ...
                  L->val[ j ] = tmp;
                  //L_subs[t].val[ j ] = tmp; // correct index
                  //for(int tk=L_subs[t].row[ii]; tk < L_subs[t].row[ii+1]; tk++ ) {
                  //  if ( L_subs[t].col[tk] == (L->col[j] - tiles[t].a[2]) ) {
                  //    L_subs[t].val[ tk ] = tmp;
                  //  }
                  //}
                }
            ...
            //    if (L->col[j] >= tiles[t].a[2] && L->col[j] < maxcol  && L->col[j] != i) {
            //      ii = i - ti;
            //      jj = L->col[j] - tiles[t].a[2];
            //      for(int tk=L_subs[t].row[ii]; tk < L_subs[t].row[ii+1]; tk++ ) {
            //        if ( L_subs[t].col[tk] == jj ) {
            //          L_subs[t].val[ tk ] = L->val[ j ];
            //        }
            //      }
            //    }
            //  }
    ...
    //printf("ready to update sub matrices\n");
    //fflush(stdout);
  ...
  printf("%% PariLU v3.8 used %d OpenMP threads and required %d iterations, %f wall clock seconds, and an average of %f wall clock seconds per iteration as measured by omp_get_wtime()\n",
