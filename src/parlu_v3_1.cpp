...
static inline void
my_calloc2(void ** ptr, const size_t nmemb, const size_t size,
...
data_ParLU_v3_1(data_d_matrix * A, data_d_matrix * L, data_d_matrix * U, int tile)
{
  ...
  printf(
    "%% ParLU v3.1 used %d OpenMP threads and required %d iterations, %f wall clock seconds, and an average of %f wall clock seconds per iteration as measured by omp_get_wtime()\n",
