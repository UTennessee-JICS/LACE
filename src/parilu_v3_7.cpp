...
data_PariLU_v3_7( data_d_matrix* A, data_d_matrix* L, data_d_matrix* U, int tile )
{
  ...
  while ( step > tol && iter < 10000 ) {
    ...
    {
    #pragma omp task
    #pragma omp parallel private(ii, jj, ti, tj, maxrow, maxcol, tmp, span, sum, infol, infou, infospmm ) num_threads(nthreads1)
    {
    ...
    #pragma omp task
    #pragma omp parallel private(ii, jj, ti, tj, maxrow, maxcol, tmp, span, sum, infol, infou, infospmm ) num_threads(nthreads2)
    {
      //#pragma omp for schedule(static,1) reduction(+:step) nowait
      //#pragma omp for reduction(+:step) nowait
  ...
  printf("%% PariLU v3.7 used %d OpenMP threads and required %d iterations, %f wall clock seconds, and an average of %f wall clock seconds per iteration as measured by omp_get_wtime()\n",
