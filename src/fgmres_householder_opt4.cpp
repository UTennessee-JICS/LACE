  ...
  data_d_gmres_log * gmres_log)
{
  ...
  for (int search = 0; search < search_max; ++search) {
    ...
    for (int j = 0; j <= search; ++j) {
      ...
      DEV_PRINTF("%s %d n=%d, j=%d, startStrip=%d BINS=%d endStrip=%d\n",
        __FILE__, __LINE__, n, j, startStrip, BINS, endStrip);
      #pragma omp parallel
      {
        #pragma omp for schedule(monotonic:static) nowait
        for (int ii = startStrip; ii < endStrip; ii += STRIP) {
    ...
    if (search < n) {
      GMRESDBG("%e %e %e %e %e\n", krylov.val[idx(search, search1,
        krylov.ld)], -dd, eps, givens_cos.val[search], givens_sin.val[search])
    ...
    if (fabs(givens.val[(search1)]) < rtol || (search == (search_max - 1)) ||
      std::isfinite(givens.val[(search1)]) == 0)
    {
