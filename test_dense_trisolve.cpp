...
int main(int argc, char* argv[])
{
  ...
  cblas_dgemv( cblas_order_const(L.major), cblas_trans_const(trans), L.num_rows, L.num_cols,
    one, L.val, L.ld, y_expect.val, 1, zero, rhs_vector.val, 1 );
...
//------------------------------------------------------------------------------  
  ...
  data_partrsv_dot( MagmaRowMajor, MagmaUpper, Magma_DENSEU, Magma_VALUE,
    U.num_rows, U.val, U.ld, rhs_vector.val, 1, x.val, 1, 
    ptrsv_tol, &ptrsv_iter );
  wcsrtrsvend = omp_get_wtime();
  ...
  cblas_dgemv( cblas_order_const(L.major), cblas_trans_const(trans), U.num_rows, U.num_cols,
    one, U.val, U.ld, x.val, 1, zero, Ax, 1 );
