...
main(int argc, char * argv[])
{
  ...
  *  ---------------------------------------------------------------------------*/
  ...
  if (argc < 4) {
    printf("Usage %s <matrix> <rhs vector> <output directory> [UNITDIAG] [restart] [maxiter]\n", argv[0]);
    return 1;
  } else {
  ...
  *  ---------------------------------------------------------------------------*/
  ...
  tmp =
    (double *) calloc( ( N * (2 * user_restart + 1) + (user_restart * (user_restart + 9)) / 2 + 1 ), sizeof(double) );
